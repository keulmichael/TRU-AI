...
class ScientificService:
    ...
    def _project_result(
        ...
        *,
        ...
        human_readable = self._human_readable(result) if (
            request.options.include_human_readable
        ) else None
    ...
    @staticmethod
    def _summary(result: ReasoningResult) -> ScientificSummary:
        return ScientificSummary(
            ...
            revision_recommendation_count=len(
    ...
    def _human_readable(
        result: ReasoningResult,
    ) -> ScientificHumanReadable | None:
        if result.synthesis is None:
            return None
        return ScientificHumanReadable(overview=result.synthesis)
